...
def state_tran_0(p1, p2):
    result = [[
                p1.policy[0]*p2.policy[0],
                p1.policy[0]*(1-p2.policy[0]),
                (1-p1.policy[0])*p2.policy[0],
                (1-p1.policy[0])*(1-p2.policy[0])
             ]]
    return torch.t(torch.tensor(result))
...
def player1ValueFunction(p1, p2):
    ...
    p_0 = state_tran_0(p1, p2)
    
    inv = torch.inverse(torch.eye(4) - gamma*P[:, 1:])
    return torch.mm(torch.mm(torch.t(p_0), inv), payoff1)
...
def player2ValueFunction(p1, p2):
    ...
    inv = torch.inverse(torch.eye(4) - gamma*P[:, 1:])
    return torch.mm(torch.mm(torch.t(p_0), inv), payoff2)
...
for epoch in range(1000):
    ...
    grad_player1_12 = torch.stack([torch.autograd.grad(grad_player1_1[i], p2.unnormal_policy, retain_graph=True)[0] for i in range(grad_player1_1.size(0))]) 
    grad_player1_2 = torch.autograd.grad(player1ValueFunction(p1, p2), p2.unnormal_policy, create_graph=True)[0]

    # According to equation 4.2
    update_grad_p1 = grad_player1_1 * delta + torch.mm(torch.t(torch.unsqueeze(grad_player1_2, dim=1)), grad_player1_12)*delta*eta


    # Doing Update for player 2
    grad_player2_2 = torch.autograd.grad(player2ValueFunction(p1, p2), p2.unnormal_policy, create_graph=True)[0]

    grad_player2_12 = torch.stack([torch.autograd.grad(grad_player2_2[i], p1.unnormal_policy, retain_graph=True)[0] for i in range(grad_player2_2.size(0))]) 
    grad_player2_1 = torch.autograd.grad(player2ValueFunction(p1, p2), p1.unnormal_policy, create_graph=True)[0]

    # According to equation 4.2
    update_grad_p2 = grad_player2_2 * delta + torch.mm(torch.t(torch.unsqueeze(grad_player2_1, 1)), grad_player2_12)*delta*eta
